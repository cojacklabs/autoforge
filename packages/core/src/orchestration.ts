...
export interface OrchestrationRepository {
  ...
  state: {
    read(): Promise<{ state: { revision: number; data: OrchestrationState } }>;
    write(
      data: OrchestrationState,
      options?: { expectedRevision?: number },
    ): Promise<{ data: OrchestrationState }>;
  };
  writePacket(packet: OrchestrationAssignmentPacket): Promise<string>;
  readPacket(assignmentId: string): Promise<OrchestrationAssignmentPacket>;
  writeHandoff(
    assignmentId: string,
...
export function scopesOverlap(left: WorkScope, right: WorkScope): boolean {
...
export function planFromWorkState(
  work: WorkStateProjection,
