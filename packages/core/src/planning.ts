...
import { triageIntentSchema, type TriageIntent } from "./intent.js";
...
function toStoryFragment(requirement: string): string {
  ...
  return acronym || trimmed.length === 0
    ? trimmed
...
export function isPlanningArtifactFresh(
  ...
  return (
    artifact.sourceFingerprint === fingerprint(triageIntentSchema.parse(input))
  );
}
