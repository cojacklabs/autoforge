...
const workflowStageIdSchema = z.string().regex(/^[a-z][a-z0-9-]*$/);
export const workflowHandoffSchema = z
  .object({
    protocolVersion: z.literal("1").default("1"),
    workflowId: z.string().min(1),
    workflowKind: z.string().min(1),
    fromStage: workflowStageIdSchema,
    toStage: workflowStageIdSchema,
...
export function createWorkflowHandoff(
  input: Omit<WorkflowHandoff, "protocolVersion" | "createdAt">,
  now = new Date(),
): WorkflowHandoff {
