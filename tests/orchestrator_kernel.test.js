...
test("OrchestratorKernel - dryRun and startRun lifecycle", async (t) => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "autoforge-kernel-test-"));
...
`
  ...
  const kernel = new OrchestratorKernel({ projectRoot: tempDir, runStore: store });
  ...
  await t.test("should start run, persist WorkItem & Run, and fetch status", () => {
