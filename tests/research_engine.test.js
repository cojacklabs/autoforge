...
test("ResearchEngine - risk scan and readiness artifact generation", async (t) => {
  const tempDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "autoforge-research-test-"),
  );
  ...
  await t.test(
    "should identify elevated risk triggers from dependencies and goals",
    async () => {
      const report = await engine.scan(
        "Build SaaS subscription checkout with AI recommendations",
      );
  ...
  await t.test("should scaffold readiness artifacts into docs/", async () => {
    ...
    assert.ok(
      fs.existsSync(
        path.join(tempDir, "docs", "security", "APPLICATION_RISK_PROFILE.md"),
      ),
    );
    assert.ok(
      fs.existsSync(
        path.join(tempDir, "docs", "privacy", "DATA_INVENTORY.yaml"),
      ),
    );
    assert.ok(
      fs.existsSync(path.join(tempDir, "docs", "security", "THREAT_MODEL.md")),
    );
    assert.ok(
      fs.existsSync(
        path.join(tempDir, "docs", "uiux", "ACCESSIBILITY_PLAN.md"),
      ),
    );
  });
