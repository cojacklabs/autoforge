...
export class Coordinator {
  constructor({ repoRoot = process.cwd(), governance = { mode: "strict" } } = {}) {
  ...
  async validateArtifact(artifact, schemaName) {
    ...
    if (!ajv) {
      this.log({ type: "warn", msg: "Ajv not installed; skipping schema validation" });
      return { valid: true, errors: [] };
    }
  ...
  async fromPrompt(rawText, options = {}) {
    ...
    const { apply = false, approvalGranted = false, formatWrite = false } = options;
    ...
    this.log({ stage: "user_ask", valid: vUserAsk.valid, errors: vUserAsk.errors });
    this.setDagStatus("user_ask", vUserAsk.valid ? "completed" : "blocked");
    await this.writeMemoryDelta("pm", `Captured UserAsk (${userAsk.id}) with intent=${userAsk.intent}`);

    // PM triage
    await this.readMemoryGate("pm");
    const issueReport = triageIssue(userAsk);
    const vIssue = await this.validateArtifact(issueReport, "IssueReport.v1.json");
    this.log({ stage: "issue_report", valid: vIssue.valid, errors: vIssue.errors });
    this.setDagStatus("issue_report", vIssue.valid ? "completed" : "blocked");
    await this.writeMemoryDelta("pm", `Triaged IssueReport (${issueReport.id}) category=${issueReport.category}`);
    ...
    if (!granted && this.governance?.mode === "strict") {
      const approvalId = this.requestApproval({ action: needApprovalKey, reason: "UI/UX inspiration research requested", scope: issueReport.components || [] });
      this.log({ stage: "approval_requested", id: approvalId, action: needApprovalKey });
      // Check if a prior decision exists for this action
      const prior = Object.values(this.approvals).find((a) => a.action === needApprovalKey && a.granted === true);
      granted = !!prior;
    }
    const allowedRoots = this.codeTargets.map((t) => t.abs);
    const { designSpec, styleGuideDiff, codePlan, testPlan } = researchAndPropose(issueReport, { approvalGranted: granted, repoRoot: this.repoRoot, allowedRoots });
    ...
    const vDesign = await this.validateArtifact(designSpec, "DesignSpec.v1.json");
    const vStyle = await this.validateArtifact(styleGuideDiff, "StyleGuideDiff.v1.json");
    this.log({ stage: "code_plan", valid: vCode.valid, errors: vCode.errors, artifact: codePlan });
    this.log({ stage: "test_plan", valid: vTest.valid, errors: vTest.errors, artifact: testPlan });
    this.log({ stage: "design_spec", valid: vDesign.valid, errors: vDesign.errors, artifact: designSpec });
    this.log({ stage: "style_diff", valid: vStyle.valid, errors: vStyle.errors, artifact: styleGuideDiff });
    this.setDagStatus("code_plan", vCode.valid ? "completed" : "blocked");
    this.setDagStatus("test_plan", vTest.valid ? "completed" : "blocked");
    await this.writeMemoryDelta("uiux", `Produced DesignSpec (${designSpec.id}) & StyleGuideDiff (${styleGuideDiff.id}); Proposed CodePlan (${codePlan.id}) & TestPlan (${testPlan.id}) for ${issueReport.components?.join(',') || 'root'}`);

    // Compliance self-audit (dry-run; check baseline on proposed files if present)
    const proposedFiles = (codePlan.changes || []).map((c) => path.join(this.repoRoot, c.path)).filter((p) => fs.existsSync(p));
    ...
    let parse_ok = true, prettier_ok = true, eslint_ok = true, tsc_ok = true, tests_ok = true;
    try {
      const validators = await import("./validators.js");
      if (proposedFiles.length) {
        parse_ok = await validators.parseChecks(proposedFiles.filter((f) => /\.(json|ya?ml|md)$/i.test(f)));
        prettier_ok = await validators.prettierCheck(proposedFiles, { write: false });
        eslint_ok = await validators.eslintCheck(proposedFiles, { fix: false });
      }
      tsc_ok = await validators.tscCheck(this.repoRoot);
    } catch (err) {
      this.log({ type: "warn", msg: `Validators not available: ${err.message}` });
    }
    const compliance = {
      rules_checked: ["artifacts_schema", "code_targets", "parse", "prettier", "eslint", "tsc"],
      artifacts_valid: vUserAsk.valid && vIssue.valid && vCode.valid && vTest.valid && vDesign.valid && vStyle.valid,
    ...
    this.setDagStatus("compliance", compliance.artifacts_valid && parse_ok && prettier_ok && eslint_ok && tsc_ok ? "completed" : "blocked");
    await this.writeMemoryDelta("qa", `Compliance baseline: ${JSON.stringify({ ...compliance, changed_files: compliance.changed_files.length })}`);
    // Engineer dry-run apply plan (no writes)
    const allowedRoots = this.codeTargets.map((t) => t.abs);
    const engPlan = planDiffs(codePlan, { repoRoot: this.repoRoot, allowedRoots });
    this.log({ stage: "engineer_plan", writes: engPlan.writes.length, violations: engPlan.violations.length });
    await this.writeMemoryDelta("engineer", `Prepared dry-run write plan: ${engPlan.writes.length} writes, ${engPlan.violations.length} violations`);

    // QA evaluation (stubbed)
    const qa = qaEvaluate(testPlan);
    this.log({ stage: "qa_eval", summary: { total: qa.total, pending: qa.pending } });
    await this.writeMemoryDelta("qa", `QA evaluation (stub): total=${qa.total}, pending=${qa.pending}`);

    // Optional apply step (approval-gated)
    let post = null;
    if (apply) {
      post = await this.applyChanges({ engPlan, testPlan, approvalGranted, formatWrite });
    }

    return { userAsk, issueReport, designSpec, styleGuideDiff, codePlan, testPlan, compliance, engineer: engPlan, qa, postApply: post, events: this.events };
  }
  ...
  async loadCodeTargets() {
    try {
      ...
      if (!file) {
        this.log({ type: "warn", msg: "Managed code_targets.yaml not found; code boundary checks disabled." });
        this.codeTargets = [];
        return;
      }
      ...
      this.log({ type: "warn", msg: `Failed to load code targets: ${err.message}` });
      this.codeTargets = [];
    }
  }
  ...
  async loadPoliciesAsync() {
    try {
      ...
      this.log({ stage: "policies_loaded", session: !!this.sessionPolicy, governance: !!this.governancePolicy });
    } catch (err) {
      this.log({ type: "warn", msg: `Failed to load policies: ${err.message}` });
    }
  }
  ...
  requestApproval({ action, reason, scope }) {
    ...
    this.events.push({ ts: new Date().toISOString(), type: "approval_request", id, action, reason, scope });
    return id;
  }

  recordApprovalDecision(id, { granted, note, action, role = "pm" }) {
    const decision = { id, action, granted: !!granted, note: note || "", ts: new Date().toISOString() };
    this.approvals[id] = decision;
    this.events.push({ ts: decision.ts, type: "approval_decision", ...decision });
  ...
  enforceCodeBoundaries(codePlan) {
    ...
    for (const change of codePlan.changes) {
      ...
      if (!insideAny) {
        violations.push({ path: change.path, allowed_roots: this.codeTargets.map((t) => path.relative(this.repoRoot, t.abs)) });
      }
    }
    return { ok: violations.length === 0, violations };
  }

  async applyChanges({ engPlan, testPlan, approvalGranted = false, formatWrite = false }) {
    // Governance: in strict mode, require explicit approval for writes
    if (this.governance?.mode === "strict" && !approvalGranted) {
      this.log({ type: "halt", msg: "Write denied: strict mode without approval" });
      return { applied: false, reason: "approval_required" };
    }
    // Session policy: allowApply must be true
    if (!this.getPermission("allowApply", true)) {
      this.log({ type: "halt", msg: "Write denied by session policy (allowApply=false)" });
    ...
    const styleEvent = this.events.reverse().find((e) => e.stage === "style_diff");
    this.events.reverse();
    let results = [];
    if (styleEvent && styleEvent.artifact && Array.isArray(styleEvent.artifact.tokenChanges)) {
      // Structured token updates for CSS variables and JSON/TS/JS theme files
      const tokenWrites = engPlan.writes.filter((w) => /\.(s?css|json|ts|js)$/i.test(w.relPath));
        ...
        try {
          ...
          if (ext === ".json") {
            ...
            if (changed > 0) {
              ...
              results.push({ path: w.relPath, action: "replace_tokens_json", ok: true, changes: changed });
            }
            ...
            if (changed > 0) {
              ...
              results.push({ path: w.relPath, action: "replace_tokens_css", ok: true, changes: changed });
            }
            ...
            for (const tc of styleEvent.artifact.tokenChanges) {
              ...
              if (re1.test(content)) { content = content.replace(re1, `$1${tc.to}$3`); changed++; }
              else if (re2.test(content)) { content = content.replace(re2, `$1${tc.to}$3`); changed++; }
            }
            if (changed > 0) {
              fs.writeFileSync(abs, content, "utf8");
              results.push({ path: w.relPath, action: "replace_tokens_tsjs", ok: true, changes: changed });
            }
          }
        } catch (err) {
          results.push({ path: w.relPath, action: "replace_tokens", ok: false, error: err.message });
        }
      }
      // Fallback for any remaining planned writes
      const remaining = engPlan.writes.filter((w) => !results.find((r) => r.path === w.relPath));
      if (remaining.length) {
        const fallback = engineer.applyWrites({ writes: remaining }, { fs, dryRun: false });
    ...
    const changedFiles = engPlan.writes.map((w) => path.resolve(this.repoRoot, w.relPath));
    let parse_ok = true, prettier_ok = true, eslint_ok = true, tsc_ok = true, tests_ok = true;
    try {
      const validators = await import("./validators.js");
      parse_ok = await validators.parseChecks(changedFiles.filter((f) => /\.(json|ya?ml|md)$/i.test(f)));
      prettier_ok = await validators.prettierCheck(changedFiles, { write: formatWrite });
      eslint_ok = await validators.eslintCheck(changedFiles, { fix: false });
      tsc_ok = await validators.tscCheck(this.repoRoot);
    } catch (err) {
      this.log({ type: "warn", msg: `Validators not available post-apply: ${err.message}` });
    }
    // Optional tests via qualityPolicies.tests.cmd
    const targetPaths = Array.isArray(testPlan?.cases)
      ? testPlan.cases.flatMap((c) => Array.isArray(c.target_paths) ? c.target_paths : [])
      : [];
    ...
    const postCompliance = {
      ...
      summary: tests_ok && parse_ok && prettier_ok && eslint_ok && tsc_ok ? "All post-apply gates passed" : "Post-apply gates failed",
    };
    this.log({ stage: "post_compliance", postCompliance });
    await this.writeMemoryDelta("qa", `Post-apply gates: ${JSON.stringify({ ok: tests_ok && parse_ok && prettier_ok && eslint_ok && tsc_ok })}`);
    // Component-level minimal diff: add focus-visible hints for Button.tsx if present
    const buttonWrites = engPlan.writes.filter((w) => /Button\.(t|j)sx$/i.test(w.relPath));
    for (const w of buttonWrites) {
      try {
        ...
        if (/className=\"[^\"]*\"/.test(content)) {
          content = content.replace(/className=\"([^\"]*)\"/, (m, g1) => `className=\"${g1} focus:outline-none focus-visible:outline-2\"`);
          fs.writeFileSync(abs, content, "utf8");
          results.push({ path: w.relPath, action: "inject_focus_classes", ok: true });
        } else if (!/AUTOFORGE: focus-visible/.test(content)) {
          content += `\n// AUTOFORGE: focus-visible outline ensure at render site`;
          fs.writeFileSync(abs, content, "utf8");
          results.push({ path: w.relPath, action: "append_focus_hint", ok: true });
        }
      } catch (err) {
        results.push({ path: w.relPath, action: "inject_focus_classes", ok: false, error: err.message });
      }
    }
  ...
  detectTestRunner(targetPaths = []) {
    try {
      ...
      const hasJest = (pkg.devDependencies && pkg.devDependencies.jest) || (pkg.dependencies && pkg.dependencies.jest) || (pkg.scripts && pkg.scripts.test && pkg.scripts.test.includes("jest"));
      const hasVitest = (pkg.devDependencies && pkg.devDependencies.vitest) || (pkg.scripts && pkg.scripts.test && pkg.scripts.test.includes("vitest"));
      const filesArg = targetPaths && targetPaths.length ? targetPaths.map((p) => `'${p}'`).join(" ") : "";
      if (hasJest) {
        // Use findRelatedTests when we have files, else run jest quietly
        return filesArg ? `npx jest --runInBand --findRelatedTests ${filesArg}` : "npx jest --runInBand";
      }
      if (hasVitest) {
  ...
  async readMemoryGate(role) {
    try {
      ...
      const g = fs.existsSync(globalPath) ? fs.readFileSync(globalPath, "utf8").slice(0, 2000) : "";
      const r = fs.existsSync(rolePath) ? fs.readFileSync(rolePath, "utf8").slice(0, 2000) : "";
      this.log({ stage: "memory_read", role, global_excerpt: g.length, role_excerpt: r.length });
    } catch (err) {
